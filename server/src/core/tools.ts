...
function currentRevision(store: WorkspaceStore) {
  return store.revision();
}

function requireCurrentWorkspace(store: WorkspaceStore): void {
...
function existingCreatedAt(store: WorkspaceStore, type: AssetType, id: string): string {
...
function appendTurn(store: WorkspaceStore, input: {
  id?: string;
  sessionId: string;
  roundId?: string;
...
function migrateV10Asset(store: WorkspaceStore, type: "goal" | "slice" | "evidence" | "unknown", raw: Record<string, unknown>) {
  const now = store.now();
  if (type === "goal") {
...
function scanProjectIndex(store: WorkspaceStore): { packages: unknown[]; docs: unknown[]; notes: string[]; top_dirs: string[] } {
...
export const IOAYN_TOOLS: IoaynTool[] = [
    ...
    execute: async (input: any, store) => {
      ...
      for (const type of ["goal", "slice", "evidence", "unknown"] as const) {
        ...
        for (const raw of values) {
          ...
          store.atomicWriteJson(path, migrateV10Asset(store, type, raw as Record<string, unknown>));
  ...
  },
  {
    ...
    inputSchema: z.object({}),
    execute: async (input: any, store) => textResult(store.getSnapshot()),
  },
  {
    name: "create_goal",
    ...
    execute: async (input: any, store) => {
      ...
      const goal = goalSchema.parse({
        ...
        created_at: existingCreatedAt(store, "goal", id),
        updated_at: store.now(),
      });
  ...
  },
  {
    ...
    execute: async (input: any, store) => {
      requireCurrentWorkspace(store);
      const participantIds = new Set(input.participants.map((participant: any) => participant.id));
      ...
      const value = sliceSchema.parse({
        ...
        created_at: existingCreatedAt(store, "slice", input.id),
        updated_at: store.now(),
      });
  ...
  },
  {
    ...
    execute: async (input: any, store) => {
      ...
      const value = unknownSchema.parse({
        ...
        created_at: existingCreatedAt(store, "unknown", id),
        updated_at: store.now(),
      });
  ...
  },
  {
    ...
    execute: async (input: any, store) => {
      ...
      const initialTurn = appendTurn(store, {
  ...
  },
  {
    ...
    execute: async (input: any, store) => {
      ...
      return textResult(appendTurn(store, input));
    },
  },
  {
  ...
  },
  {
    ...
    execute: async (input: any, store) => {
      ...
      try {
        ...
        const userTurns = input.userTurns.map((turn: any, index: number) =>
          appendTurn(store, {
        ...
        const agentTurns = input.agentTurns.map((turn: any, index: number) =>
          appendTurn(store, {
        ...
        const evidence: Evidence[] = input.evidence.map((item: any, index: number) => {
        ...
        const unknowns: Unknown[] = input.unknowns.map((item: any, index: number) => {
        ...
        const evidenceRefs = unique([
          ...
          ...input.claims.flatMap((claim: any) => claim.evidenceRefs),
        ]);
        const unknownRefs = unique(unknowns.map((item) => item.id));
        const claims = input.claims.map((claim: any, index: number) =>
          claimSchema.parse({
        ...
        if (input.learningAsset) {
          ...
          learningAsset = learningAssetSchema.parse({
            ...
            source_turn_refs: [...userTurns, ...agentTurns].map((turn: any) => turn.id),
            ...
            revision,
            created_at: existingCreatedAt(store, "learning_asset", assetId),
            updated_at: store.now(),
          });
          ...
          for (const entity of input.introducedEntities.filter((item: any) => item.map)) {
        ...
        const round = roundSchema.parse({
          ...
          summary: input.summary,
          user_turn_refs: userTurns.map((turn: any) => turn.id),
          agent_turn_refs: agentTurns.map((turn: any) => turn.id),
          introduced_entities: input.introducedEntities,
          claims,
          evidence_refs: evidenceRefs,
          unknown_refs: unknownRefs,
          ...
          revision,
          created_at: existingCreatedAt(store, "round", input.roundId),
          updated_at: store.now(),
        });
        ...
        appendTurn(store, {
          ...
          relatedEntities: input.introducedEntities.map((entity: any) => entity.id),
  ...
  {
    name: "find_historical_connections",
  ...
  },
  {
    ...
    inputSchema: z.object({}),
    execute: async (input: any, store) => {
      const scanned = scanProjectIndex(store);
      const value = {
        revision: currentRevision(store),
        built_at: store.now(),
        ...scanned,
